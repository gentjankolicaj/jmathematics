...
import io.mathematics.modular.CommonResidue;

/**
 * Implementation of <a href="https://en.wikipedia.org/wiki/Extended_Euclidean_algorithm">Extended Euclidean
 * Algorithm</a>
 */
public class ExtendedEuclideanAlgorithm implements Algorithm {
  ...
  public static int mulInv(final int a, final int b) {
    if (EuclideanAlgorithm.gcd(a, b) != 1) {
      throw new OperationException("Multiplicative inverse of " + a + " & " + b + " not found, a & b are not coprime.");
    }
    int r0 = Math.max(a, b);
    int r1 = Math.min(a, b);
    int ri;
  ...
  public static long mulInv(final long a, final long b) {
    if (EuclideanAlgorithm.gcd(a, b) != 1) {
      throw new OperationException("Multiplicative inverse of " + a + " & " + b + " not found, a & b are not coprime.");
    }
    long r0 = Math.max(a, b);
